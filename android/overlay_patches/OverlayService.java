...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        clampOverlayPosition((WindowManager.LayoutParams) flutterView.getLayoutParams());
        windowManager.updateViewLayout(flutterView, (WindowManager.LayoutParams) flutterView.getLayoutParams());
        return START_STICKY;
    }
    ...
    /** Cập nhật kích thước màn hình (xoay ngang/dọc trên màn xe). */
    ...
    /**
     * Giữ nút nổi trong vùng hiển thị — tránh trôi lên mép trên/dưới khi xoay màn hình.
     * positionGravity.auto chỉ snap trái/phải; cần clamp Y (và X) trong lúc kéo và sau thả tay.
     */
    private void clampOverlayPosition(WindowManager.LayoutParams params) {
        ...
        int edgeMargin = dpToPx(12);
        int topInset = statusBarHeightPx() + edgeMargin;
        int bottomInset = (inPortrait() ? navigationBarHeightPx() : 0) + edgeMargin;
        int gravity = params.gravity;

        // Ngang: với gravity RIGHT, x là khoảng cách từ mép phải màn hình.
        if ((gravity & Gravity.RIGHT) != 0 && (gravity & Gravity.LEFT) == 0) {
            int minX = 0;
            int maxX = Math.max(minX, screenW - viewWidth);
        ...
        // Dọc: centerRight dùng CENTER_VERTICAL → y tính từ giữa màn hình.
        int verticalGravity = gravity & Gravity.VERTICAL_GRAVITY_MASK;
        if (verticalGravity == Gravity.CENTER_VERTICAL) {
            int minY = topInset - screenH / 2 + viewHeight / 2;
            int maxY = screenH - bottomInset - screenH / 2 - viewHeight / 2;
            if (minY > maxY) {
                minY = maxY = params.y;
            }
            ...
            int minY = topInset;
            int maxY = Math.max(minY, screenH - viewHeight - bottomInset);
            if (params.y < minY) params.y = minY;
            if (params.y > maxY) params.y = maxY;
        } else if ((gravity & Gravity.BOTTOM) != 0) {
            int minY = -(screenH - viewHeight - bottomInset);
            int maxY = -topInset;
            if (params.y < minY) params.y = minY;
            if (params.y > maxY) params.y = maxY;
        }
